...
// CARGA DE TABLA DINAMICA
 ...
 $('.tablaMovimientosSalida').DataTable( {
    "ajax": "ajax/datatable-movimientos-salida.ajax.php",
    "deferRender": true,
	"retrieve": true,
	"processing": true,
	 "language": {
...
$(".tablaMovimientos").on("draw.dt", function(){
	...
	if(localStorage.getItem("quitarProducto2") != null){
		...
		for(var i = 0; i< listaIdProducto.length; i++){
			...
			$("button.recuperarBoton3[idProducto='"+listaIdProducto[i]["idProducto"]+"']").removeClass('btn-default');
			$("button.recuperarBoton3[idProducto='"+listaIdProducto[i]["idProducto"]+"']").addClass('btn-primary agregarProducto');
		}
...
$(".formularioMovimientos").on("click", "button.quitarProducto2", function(){
	...
	$("button.recuperarBoton3[idProducto='"+idProducto+"']").removeClass('btn-default');

	$("button.recuperarBoton3[idProducto='"+idProducto+"']").addClass('btn-primary agregarProducto');
...
// MODIFICAR LA CANTIDAD DE PRODUCTOS EN SALIDA
...
$('#daterangeMovimientos-btn').daterangepicker(
	...
	function (start, end) {
	  ...
	  var capturarRango = $("#daterangeMovimientos-btn span").html();
		 ...
		 window.location = "index.php?ruta=movimientos-inventario&fechaInicial="+fechaInicial+"&fechaFinal="+fechaFinal;
...
$(".daterangepicker.openscenter .range_inputs .cancelBtn").on("click", function(){
	...
	window.location = "movimientos-inventario";
